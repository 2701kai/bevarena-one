...
extract_bullet_points() {
    ...
    for section in "Changes" "Features" "What's New" "Updates" "Improvements" "Fixes" "Fixed"; do
        ...
        if [[ -n "$section_start" ]]; then
            ...
            # Use sed instead of tail for better compatibility
            next_heading_line=$(sed -n "$((section_start + 1)),\$p" "$file" | grep -n "^##* " | head -1 | cut -d':' -f1)

            if [[ -n "$next_heading_line" ]]; then
                # Calculate the line number of the next heading
                next_heading=$((section_start + next_heading_line))
...
extract_summary() {
    ...
    if [[ -n "$title_line" ]]; then
        # Look for the first paragraph after the title - use proper range notation
        summary=$(sed -n "$((title_line + 1)),$((title_line + 5))p" "$file" | grep -v "^$" | grep -v "^#" | grep -v "^[-*+]" | head -3)
    fi
    ...
    if [[ -z "$summary" ]]; then
        for section in "Summary" "Overview" "Description"; do
            ...
            if [[ -n "$section_line" ]]; then
                ...
                summary=$(sed -n "$((section_line + 1)),$((section_line + 5))p" "$file" | grep -v "^$" | grep -v "^#" | grep -v "^[-*+]" | head -3)
                break
            fi
        done
    fi
